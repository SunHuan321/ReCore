...
definition p4_post_noframe :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p4_post_noframe t s st = ([A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L
      \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[ TOP]\<^sub>v =\<^sub>b [stack_top st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
lemma safe_p24_noframe : "fvAs \<Gamma> = {} \<Longrightarrow> t \<noteq> NULL \<Longrightarrow>
  ...
  \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** stack_node s st" in safe_assn_equiv, simp add: assn_equiv_reflex)
    ...
    apply (rule rule_stm_stack' , simp_all, simp add: disjoint_def)
...
lemma safe_p24 : "fvAs \<Gamma> = {} \<Longrightarrow> t \<noteq> NULL \<Longrightarrow>
  ...
  apply (simp add: p1_post_def stm_def disjoint_def )
  done

lemma p5_pre_implies : "(p1_post t ** (p4_post_noframe t s st ** p4_frame st) \<and>\<^sub>S\<^sub>L \<langle>[NEXT]\<^sub>v =\<^sub>b [TOP]\<^sub>v\<rangle>\<^sub>S\<^sub>L)
...
lemma safe_p56 : "fvAs \<Gamma> = {} \<Longrightarrow> t \<noteq> NULL \<Longrightarrow>
                 \<Gamma>(Cur := inv_cur, Readyq := inv_readyq, Stack := inv_stack) \<turnstile>
                          ...
                          { inv_stack1 s st}"
  apply (rule_tac Q = "inv_stack1 s st \<and>\<^sub>S\<^sub>L \<langle>[ RET]\<^sub>v =\<^sub>b [ENOMEM]\<^sub>n\<rangle>\<^sub>S\<^sub>L" in rule_seq)
...
definition p7_pre_noframe :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p7_pre_noframe t s st = ([A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L
          \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"

definition p7_post_noframe :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p7_post_noframe t s st = ([A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L
  \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
lemma safe_p7_noframe : "fvAs \<Gamma> = {} \<Longrightarrow> 
  ...
  apply (rule_tac Pa = "(stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (\<langle>[ NEXT]\<^sub>v =\<^sub>b 
  [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L Aemp)" and Qa = "(stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L  \<and>\<^sub>S\<^sub>L 
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (\<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L Aemp)"
  in safe_assn_equiv, simp_all add: assn_equiv_def) apply auto[1]
   apply (rule rule_frame, rule stack_node_read_wait, simp add: )
  by (simp_all add: fvA_Gamma2 fvA_inv_cur fvA_inv_stack fvA_inv_readyq disjoint_def )

definition p9_pre_noframe :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p9_pre_noframe t s a list st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L 
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** is_waitq (stack_wait st) (a # list)"

definition p9_pre :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p9_pre t s a list st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (thread_node 
  (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (thread_wait_sl (thd_next a)
  list )"
...
lemma p9_pre_implies : "p9_pre_noframe t s a list st \<sqsubseteq> p9_pre t s a list st"
  apply (rule_tac Q = "(stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b 
...
definition p9_post :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p9_post t s a list st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (thread_node 
  (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[WAITQ]\<^sub>v =\<^sub>b [thd_next a]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
...
definition p10_pre :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p10_pre t s a list st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[WAITQ]\<^sub>v =\<^sub>b [thd_next a]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
  ** (thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L ) 
  ** (thread_wait_sl (thd_next a) list)"
...
definition p10_post :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p10_post t s a list st = (stack_node s (st:=\<^sub>w\<^sub>a thd_next a) \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L 
  \<langle>[WAITQ]\<^sub>v =\<^sub>b [thd_next a]\<^sub>n\<rangle>\<^sub>S\<^sub>L)  ** (thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v 
...
definition p10_post_noframe :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> tcbs \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  list) ** (thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) "
...
lemma p10_post_implies : " p10_post t s a list st \<sqsubseteq> p10_post_noframe t s a list st"
  ...
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl (thd_next a) list" in implies_trans)
   apply (simp add: p10_post_def implies_def) apply auto[1]
  apply (rule equiv_implies, simp add: p10_post_noframe_def)
  apply (rule_tac Q = "stack_node s (st :=\<^sub>w\<^sub>a thd_next a) ** (thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L 
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** is_waitq (thd_next a) list" in assn_equiv_trans)
...
lemma safe_p910_noframe : "\<lbrakk>fvAs \<Gamma> = {}; stack_pt st; stack_next st \<noteq> stack_top st\<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac P = "p9_pre t s a list st" and Q = "p10_post t s a list st" in rule_conseq)
    apply (rule_tac Q = "p9_post t s a list st" in rule_seq, simp add: p9_pre_def p9_post_def)
     apply (rule rule_frame, rule rule_frame1, rule thread_node_readnext, simp add: )
    ...
    apply (simp add: p10_pre_def p10_post_def, intro rule_frame, rule stack_node_write_wait_with_var)
      apply (simp add: fvA_Gamma2 fvA_inv_cur fvA_inv_stack fvA_inv_readyq )
...
definition p13_pre_noframe :: "nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p13_pre_noframe t a st = (thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v 
...
lemma p9_pre_frame_implies : "stack_tcbs st = a # list \<Longrightarrow> (p1_post t ** (p7_post_noframe t s st ** p4_frame st) 
  \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L) \<sqsubseteq> p9_pre_noframe t s a list st ** p9_frame s st"
  apply (rule_tac Q = "[A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L 
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** p4_frame st" in implies_trans)
...
lemma p10_post_frame_equiv : "\<lbrakk> stack_next st \<noteq> stack_top st; stack_base st = stack_next st; 
  ...
  apply (rule_tac Q = "(thread_node (stack_wait st) a \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
...
lemma safe_p910 : "\<lbrakk>fvAs \<Gamma> = {}; stack_next st \<noteq> stack_top st; stack_base st = stack_next st; 
                 ...
                 \<Gamma>(Cur := inv_cur, Readyq := inv_readyq, Stack := inv_stack) \<turnstile>
{(p1_post t ** (p7_post_noframe t s st ** p4_frame st) \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L)}
                          WAITQ :=\<^sub>C \<lbrakk>[FIRST_PENDING_THREAD]\<^sub>v\<Down>\<^sub>tnext\<rbrakk>;;
                          \<lbrakk>[ADDR]\<^sub>v\<Down>\<^sub>swait\<rbrakk> :=\<^sub>C [WAITQ]\<^sub>v  
{p13_pre_noframe t a st ** inv_stack1 s (st :=\<^sub>w\<^sub>a thd_next a :=\<^sub>t\<^sub>c list)}"
  ...
  a list st ** p9_frame s st" in rule_conseq)
    ...
    apply (simp add: p9_frame_def fvA_buf inv_stack_pt_def inv_stack_buf_waitq_def disjoint_def )
...
definition p11_pre_noframe :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p11_pre_noframe t s st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (buf (stack_base st) 
  (stack_top st) (stack_buf st) \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
definition p11_post :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p11_post t s data st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (buf (stack_base st) 
  (stack_top st) ((stack_buf st)[(stack_next st -stack_base st) := data]) \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"

definition p12_pre :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p12_pre t s data st = (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[ NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
...
definition p12_post :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p12_post t s data st = (stack_node s (st:=\<^sub>n\<^sub>e (stack_next st + 1)) \<and>\<^sub>S\<^sub>L \<langle>[ ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L 
...
lemma safe_p1112_noframe : "\<lbrakk>fvAs \<Gamma> = {}; stack_pt st; stack_next st \<noteq> stack_top st; t \<noteq> NULL\<rbrakk> \<Longrightarrow>
  ...
  apply (rule_tac P = "p12_pre t s data st" and Q = "p12_post t s data st" in rule_conseq)
    apply (simp only: p12_post_def p12_pre_def, rule rule_frame, rule rule_stm_stack', simp, simp)
       apply (simp add: stack_node_def , simp)
  apply (simp add: stack_node_def )
     apply (rule stack_node_write_next_add1, simp add: )
...
lemma p11_pre_frame_implies : "(p7_post_noframe t s st ** p4_frame st \<and>\<^sub>S\<^sub>L\<langle>[FIRST_PENDING_THREAD]\<^sub>v 
                        =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L) \<sqsubseteq> p11_pre_noframe t s st ** p11_frame s st"
  apply (rule_tac Q = "([A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L
      \<langle>[NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)) ** p4_frame st" in implies_trans)
...
lemma safe_p1112 : "\<lbrakk>fvAs \<Gamma> = {}; stack_pt st; stack_next st \<noteq> stack_top st; stack_wait st = NULL; 
                 ...
                 \<Gamma>(Cur := inv_cur, Readyq := inv_readyq, Stack := inv_stack) \<turnstile>
           {  p7_post_noframe t s st ** p4_frame st \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L }
                (t \<^enum> \<lbrakk>[ NEXT]\<^sub>v\<rbrakk> :=\<^sub>C [data]\<^sub>n) ;;
                (t \<^enum>  \<lbrakk>[ ADDR]\<^sub>v\<Down>\<^sub>snext\<rbrakk> :=\<^sub>C [ NEXT]\<^sub>v +\<^sub>e [1]\<^sub>n)
                 { inv_stack }"
  ...
  ** p11_frame s st" in rule_conseq, rule rule_frame, rule safe_p1112_noframe, simp_all)
    ...
    apply (simp add: inv_stack_buf_waitq_def)
...
definition p14_post_noframe :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
definition p13_post :: "nat  \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
lemma safe_p1314_noframe : "\<lbrakk>fvAs \<Gamma> = {}; stack_pt st; stack_next st \<noteq> stack_top st\<rbrakk> \<Longrightarrow>
              ...
              \<lbrakk>[ FIRST_PENDING_THREAD]\<^sub>v\<Down>\<^sub>tdata\<rbrakk> :=\<^sub>C [d]\<^sub>n
...
lemma safe_p1314 : "\<lbrakk>fvAs \<Gamma> = {}; stack_pt st; stack_next st \<noteq> stack_top st\<rbrakk> \<Longrightarrow>
   ...
   {p13_pre_noframe t a st ** inv_stack1 s (st :=\<^sub>w\<^sub>a thd_next a :=\<^sub>t\<^sub>c list) \<and>\<^sub>S\<^sub>L 
            \<langle>\<not>\<^sub>b [FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L}
              \<lbrakk>[ FIRST_PENDING_THREAD]\<^sub>v\<Down>\<^sub>tstate\<rbrakk> :=\<^sub>C [READY]\<^sub>n ;;
              \<lbrakk>[ FIRST_PENDING_THREAD]\<^sub>v\<Down>\<^sub>tdata\<rbrakk> :=\<^sub>C [d]\<^sub>n
...
definition p15_post :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> nat \<Rightarrow> tcbs \<Rightarrow> assn"
  ...
  \<langle>[FIRST_READY]\<^sub>v =\<^sub>b [r]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** is_readyq r xs)"
...
definition p16_pre :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> nat \<Rightarrow> tcbs \<Rightarrow> assn"
  ...
  \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[FIRST_READY]\<^sub>v =\<^sub>b [r]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (([A_Readyq]\<^sub>v
...
definition p17_pre :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> nat \<Rightarrow> tcbs \<Rightarrow> assn"
  ...
  ([A_Readyq]\<^sub>v  \<longmapsto> [r]\<^sub>n \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** is_readyq r xs)"
...
definition p17_post :: "nat \<Rightarrow> nat \<Rightarrow> tcb \<Rightarrow> stack_mem \<Rightarrow> nat \<Rightarrow> tcbs \<Rightarrow> assn"
  ...
  ([A_Readyq]\<^sub>v  \<longmapsto> [stack_wait st]\<^sub>n \<and>\<^sub>S\<^sub>L \<langle>[FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** is_readyq r xs)"
...
lemma safe_p1517_noframe : "fvAs \<Gamma> = {} \<Longrightarrow> 
  ...
  apply (rule_tac P = "[A_Readyq]\<^sub>v \<longmapsto> [r]\<^sub>n" and Q = "[A_Readyq]\<^sub>v \<longmapsto> [r]\<^sub>n \<and>\<^sub>S\<^sub>L \<langle>[FIRST_READY]\<^sub>v =\<^sub>b 
  [r]\<^sub>n\<rangle>\<^sub>S\<^sub>L" in rule_conseq, rule rule_read, simp add: fvA_Gamma2 fvA_inv_cur fvA_inv_stack fvA_inv_stack)
         apply (simp add: fvA_inv_readyq , simp add: implies_def, simp add: implies_def)
...
lemma safe_p1517 : "fvAs \<Gamma> = {} \<Longrightarrow> 
  ...
  ((a :=\<^sub>s\<^sub>t READY :=\<^sub>d\<^sub>t d :=\<^sub>n\<^sub>x r) # xs)" in rule_conseq)
    ...
    apply (simp add: disjoint_def ) apply (rule equiv_implies) 
...
lemma safe_push : "fvAs \<Gamma> = {} \<Longrightarrow> 
  ...
  apply (simp only: stack_push_def) 
  apply (rule rule_Clocals)
  apply (case_tac t, simp add: CSL_def) 
  ...
  apply (case_tac "stack_wait st")
   apply (rule_tac Q = "p7_post_noframe t s st ** p4_frame st" in rule_seq)
         apply (rule rule_stm_stack', simp_all, simp add: disjoint_def )
            ...
            apply (simp add: stack_node_def fvA_is_waitq fvA_buf inv_stack_buf_waitq_def )
           apply (simp add: p4_frame_def inv_stack_pt_def p1_post_def p7_pre_noframe_def)
           apply (simp add: stack_node_def fvA_is_waitq fvA_buf inv_stack_buf_waitq_def )
    apply (rule_tac Q = "p1_post t ** (p7_post_noframe t s st ** p4_frame st)" in rule_seq)
     ...
     apply (simp add: p1_post_def, simp add: disjoint_def )
...
(* stack_wait st \<noteq>  NULL *)
    ...
    using safe_p910 apply auto[1] 
     apply (rule rule_if)
     apply (simp add: p13_pre_noframe_def CSL_def) using stm_def apply auto[1]
      apply (rule rule_stm_stack', simp_all)
          apply (simp add: disjoint_def )
         apply (simp add: p13_pre_noframe_def inv_stack1_def fvA_is_stack )
    apply (simp add: p13_pre_noframe_def inv_stack1_def fvA_is_stack )
   ...
   apply (rule rule_if, rule_tac P = Aemp and Q = "Aemp \<and>\<^sub>S\<^sub>L \<langle>[RET]\<^sub>v =\<^sub>b [0]\<^sub>n\<rangle>\<^sub>S\<^sub>L"  in rule_conseq)
       apply (rule rule_stm_stack', simp_all)
    apply (simp add: disjoint_def )
        apply (rule rule_assign_num1, simp add: fvA_Gamma2 fvA_inv_stack fvA_inv_cur fvA_inv_readyq)
