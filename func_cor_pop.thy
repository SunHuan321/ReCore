...
definition p4_post_noframe :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p4_post_noframe t s st = ([A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L
      \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> BASE]\<^sub>v =\<^sub>b [stack_base st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
definition p5_post :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p5_post t s st = (stack_node s (st:=\<^sub>n\<^sub>e (stack_next st - 1)) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L 
...
definition p6_pre :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (buf (stack_base st) (stack_top st) (stack_buf st))"
...
definition p6_post :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p6_post t s st = (stack_node s (st:=\<^sub>n\<^sub>e (stack_next st - 1)) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L
  \<langle>[t \<diamondop> NEXT]\<^sub>v =\<^sub>b [stack_next (st:=\<^sub>n\<^sub>e (stack_next st - 1))]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** 
  (buf (stack_base st) (stack_top st) (stack_buf st))"

definition p7_pre' :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p7_pre' t s st = (stack_node s (st:=\<^sub>n\<^sub>e (stack_next st - 1)) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> NEXT]\<^sub>v =\<^sub>b 
...
definition p7_pre :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  (stack_top st) (stack_buf st) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> NEXT]\<^sub>v =\<^sub>b [stack_next st - 1]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
definition p7_post :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  (stack_top st) (stack_buf st) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> NEXT]\<^sub>v =\<^sub>b [stack_next st - 1]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L 
  \<langle>[t \<diamondop> DATA]\<^sub>v =\<^sub>b [(stack_buf st) ! (stack_next st - 1 - stack_base st)]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
definition p8_pre :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  (stack_top st) (stack_buf st)  \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> DATA]\<^sub>v =\<^sub>b [(stack_buf st) ! (stack_next st 
...
definition p8_post :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p8_post t s st = (p8_pre t s st \<and>\<^sub>S\<^sub>L \<langle>[(t \<diamondop> RET)]\<^sub>v =\<^sub>b [0]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"

definition p9_post :: "nat \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p9_post t s st = (p8_post t s st \<and>\<^sub>S\<^sub>L \<langle>[(t \<diamondop> END)]\<^sub>v =\<^sub>b [1]\<^sub>n\<rangle>\<^sub>S\<^sub>L)"
...
lemma p5_pre_implies : " (p4_post_noframe t s st \<and>\<^sub>S\<^sub>L \<langle>[t\<diamondop>NEXT]\<^sub>v >\<^sub>b [t\<diamondop>BASE]\<^sub>v\<rangle>\<^sub>S\<^sub>L) ** p4_frame st 
                          \<sqsubseteq> p5_pre_noframe t s st ** p5_frame s st"
  apply (rule_tac Q = "[A_Stack]\<^sub>v \<longmapsto> [s]\<^sub>n ** (stack_node s st \<and>\<^sub>S\<^sub>L  \<langle>[t\<diamondop>ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L 
  \<and>\<^sub>S\<^sub>L \<langle>[t\<diamondop>NEXT]\<^sub>v =\<^sub>b [stack_next st]\<^sub>n\<rangle>\<^sub>S\<^sub>L ) ** p4_frame st" in implies_trans)
...
definition p12_pre_noframe :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p12_pre_noframe t x s st = (inv_cur1 t x) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
...
definition p12_post :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p12_post t x s st = (inv_cur1 t x) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L \<and>\<^sub>S\<^sub>L 
  \<langle>[t \<diamondop> FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl (stack_wait st) (stack_tcbs st)"

definition p13_pre' :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p13_pre' t x s st = (inv_cur1 t x \<and>\<^sub>S\<^sub>L  \<langle>[t \<diamondop> FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b 
  [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl 
  (stack_wait st) (stack_tcbs st)"

definition p13_pre :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  where "p13_pre t x s st = (([A_Cur]\<^sub>v \<longmapsto> [t]\<^sub>n) ** ((thread_node t x) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b 
  [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl 
  (stack_wait st) (stack_tcbs st)" 
...
definition p13_post :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  \<langle>[t \<diamondop> FIRST_PENDING_THREAD]\<^sub>v =\<^sub>b [stack_wait st]\<^sub>n\<rangle>\<^sub>S\<^sub>L)) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
  ** thread_wait_sl (stack_wait st) (stack_tcbs st)"
...
definition p14_pre :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl (stack_wait st) (stack_tcbs st)"
...
definition p15_post :: "nat \<Rightarrow> tcb \<Rightarrow> nat \<Rightarrow> stack_mem \<Rightarrow> assn"
  ...
  (stack_node s (st:=\<^sub>w\<^sub>a t) \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) ** thread_wait_sl (stack_wait st) (stack_tcbs st)"
...
lemma p12_pre_frame_implies : " p4_post_noframe t s st ** p4_frame st ** inv_cur1 t x \<sqsubseteq>
  ...
  apply (rule_tac Q = "(inv_cur1 t x) ** (stack_node s st \<and>\<^sub>S\<^sub>L \<langle>[t \<diamondop> ADDR]\<^sub>v =\<^sub>b [s]\<^sub>n\<rangle>\<^sub>S\<^sub>L) 
...
lemma p5_cond_aux : "\<sigma> \<Turnstile> (p4_post_noframe t s st \<and>\<^sub>S\<^sub>L \<langle>[t\<diamondop>NEXT]\<^sub>v >\<^sub>b [t\<diamondop>BASE]\<^sub>v\<rangle>\<^sub>S\<^sub>L) ** p4_frame st
...
lemma p12_cond_aux : "\<sigma> \<Turnstile> ((p4_post_noframe t s st  ** p4_frame st) \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [t\<diamondop>NEXT]\<^sub>v >\<^sub>b [t\<diamondop>BASE]\<^sub>v\<rangle>\<^sub>S\<^sub>L) 
...
lemma p12_cond_aux1 : "user_cmd C \<Longrightarrow> stack_base st = stack_next st \<longrightarrow> \<Gamma> \<turnstile> {((p4_post_noframe t s st  ** p4_frame st) 
     \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [t\<diamondop>NEXT]\<^sub>v >\<^sub>b [t\<diamondop>BASE]\<^sub>v\<rangle>\<^sub>S\<^sub>L) ** P} C {Q} \<Longrightarrow> \<Gamma> \<turnstile> {((p4_post_noframe t s st  ** p4_frame st) 
     \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [t\<diamondop>NEXT]\<^sub>v >\<^sub>b [t\<diamondop>BASE]\<^sub>v\<rangle>\<^sub>S\<^sub>L) ** P} C {Q}"
...
lemma safe_p1819 : "t \<noteq> NULL \<Longrightarrow> fvAs \<Gamma> = {} \<Longrightarrow>
                    \<Gamma>(Cur := inv_cur, Readyq := inv_readyq, Stack := inv_stack) 
                      \<turnstile> {(Aemp \<and>\<^sub>S\<^sub>L \<langle>[t\<diamondop>END]\<^sub>v =\<^sub>b [K_NO_WAIT]\<^sub>n\<rangle>\<^sub>S\<^sub>L) \<and>\<^sub>S\<^sub>L \<langle>\<not>\<^sub>b [t\<diamondop>RET]\<^sub>v =\<^sub>b [EAGAIN]\<^sub>n\<rangle>\<^sub>S\<^sub>L  } 
  ...
  in rule_conseq, simp add: inv_cur1_def is_cur_def)
     apply (rule_tac Pa = "([A_Cur]\<^sub>v \<longmapsto> [t]\<^sub>n \<and>\<^sub>S\<^sub>L inv_is_running ta) ** thread_node t ta" and Qa = "
      ([A_Cur]\<^sub>v \<longmapsto> [t]\<^sub>n  \<and>\<^sub>S\<^sub>L inv_is_running ta) ** (thread_node t ta \<and>\<^sub>S\<^sub>L \<langle>[t\<diamondop>DATA]\<^sub>v =\<^sub>b [thd_data ta]\<^sub>n\<rangle>\<^sub>S\<^sub>L)" 
       in safe_assn_equiv)
